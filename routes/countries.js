...
app.get('/', function(req, res, next) {
	req.getConnection(function(error, conn) {
		conn.query('SELECT * FROM node_countries ORDER BY id DESC',function(err, rows, fields) {
			//if(err) throw err
			if (err) {
				req.flash('error', err)
...
app.post('/add', function(req, res, next){	
    ...
    if( !errors ) {   //No errors were found.  Passed Validation!
		...
		********************************************/
		...
		req.getConnection(function(error, conn) {
			conn.query('INSERT INTO node_countries SET ?', country, function(err, result) {
				//if(err) throw err
				if (err) {
					req.flash('error', err)
...
app.get('/edit/(:id)', function(req, res, next){
	req.getConnection(function(error, conn) {
		conn.query('SELECT * FROM node_countries WHERE id = ' + req.params.id, function(err, rows, fields) {
			if(err) throw err
...
app.put('/edit/(:id)', function(req, res, next) {
    ...
    if( !errors ) {   //No errors were found.  Passed Validation!
		...
		********************************************/
		...
		req.getConnection(function(error, conn) {
			conn.query('UPDATE node_countries SET ? WHERE id = ' + req.params.id, country, function(err, result) {
				//if(err) throw err
				if (err) {
					req.flash('error', err)
...
app.delete('/delete/(:id)', function(req, res, next) {
	...
	req.getConnection(function(error, conn) {
		conn.query('DELETE FROM node_countries WHERE id = ' + req.params.id, country, function(err, result) {
			//if(err) throw err
			if (err) {
				req.flash('error', err)
